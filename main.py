...
# db_url = f"postgresql://root:password@localhost:5432/shopping_db"
...
def run(db_url: str, lang: Query.Lang):
    ...
    def describe_relationships():
        ...
        for table_name in metadata.tables:
            table = metadata.tables[table_name]
            print(table.constraints, table.foreign_key_constraints)

            symbol = ""
            has_primary_key = any(column.primary_key for column in table.columns)
            has_foreign_key = any(
                isinstance(constraint, sqlalchemy.ForeignKeyConstraint) for constraint in table.constraints)
            has_unique_constraint = any(
                isinstance(constraint, sqlalchemy.UniqueConstraint) for constraint in table.constraints)
...
convert_svg_to_png('diagram.svg')
    ...
    DescriptionGenerator.runner(text_description, lang)
